...
async function importExcel() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    try {
      ...
      const numeroChamado = row.__EMPTY;
      const local = row.__EMPTY_1 || 'Não especificado';
      const abertura = parseExcelDate(row.__EMPTY_14);
      const prazo = parseExcelDate(row.__EMPTY_15);
      const retorno = row.__EMPTY_16 || '';
      const responsavel = normalizeResponsible(row.Responsável || '');
      const status = normalizeStatus(row.__EMPTY_13 || '');
